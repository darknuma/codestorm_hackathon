...
for region, crops in CROPS_BY_REGION.items():
	if (
		'Sorghum' in crops and 'Guinea Corn' not in crops
	):  # Ensure Guinea Corn is added if Sorghum exists
...
def calculate_yield_tons_per_ha(row):
	...
	if row['uses_fertilizer']:
		modern_practices_factor += 0.20
	if row['uses_irrigation']:
		modern_practices_factor += 0.35
...
def generate_prior_loan_details(row):
	if row['has_prior_loan']:
		...
		loan_amount = min(
			potential_revenue * random.uniform(0.10, 0.35), 200000 + (row['farm_size_ha'] * 60000)
		)
		...
		if row['cooperative_member']:
			base_repayment_prob += 0.07
		if row['has_off_farm_income']:
			base_repayment_prob += 0.08
...
for i in range(NUM_FARMERS):
	...
	if marital_status == 'Single':
		if age < 22:
			household_size = random.randint(1, 3)
		elif age < 30:
		...
		min_hh_size = 2
		if age < 22:
			household_size = random.randint(min_hh_size, min_hh_size + 2)
		elif age < 30:
			household_size = random.randint(min_hh_size, min_hh_size + 4)
		else:
			household_size = random.randint(min_hh_size, 14)
	...
	primary_crop = (
		random.choices(regional_crops, weights=crop_weights, k=1)[0]
		if regional_crops
		else 'Unknown'
	)  # Handle empty regional_crops

	secondary_crop_options = (
		[c for c in regional_crops if c != primary_crop] + ['None'] if regional_crops else ['None']
	)
	...
	if gender == 'Female':
		uses_fertilizer = uses_fertilizer if random.random() < 0.9 else False
	else:
		uses_fertilizer = uses_fertilizer if random.random() < 1.1 else True
	...
	if land_acquisition_method == 'Purchase':
		has_land_title = random.choices([True, False], weights=[0.20, 0.80])[0]
	else:
		has_land_title = random.choices([True, False], weights=[0.05, 0.95])[0]
	...
	if value_chain_platform_registered:
		...
		if years_on_platform > 0:
			marketplace_sales_ngn_last_year = random.uniform(0.1, 0.6)
	...
	total_annual_income_ngn = (
		annual_farm_revenue_ngn + annual_off_farm_income_ngn
	)  # Corrected: farm revenue + off-farm income
	...
	suitable_loan_products = determine_suitable_loan_products(
		{**full_row_for_scoring, 'farm_size_ha': farm_size_ha}
	)
...
def create_farm_enterprise_table(df_master_data):
	...
	for _, row in df_master_data.iterrows():
		...
		if row['owns_tractor']:
			equipment_type = 'Modern'
		elif row['owns_plow'] or row['owns_sprayer']:
			equipment_type = 'Integrated'
		elif row['owns_cutlass'] or row['owns_sickle']:
			equipment_type = 'Traditional'

		value_of_farm_assets = (
			(row['farm_size_ha'] * 300000)
		...
		farm_enterprises.append(
			{
				...
				'Monthly_Income_NGN': round(row['annual_farm_revenue_ngn'] / 12, 0)
				if row['annual_farm_revenue_ngn']
				else 0,
				'Monthly_Expense_NGN': round(row['annual_farm_expenses_ngn'] / 12, 0)
				if row['annual_farm_expenses_ngn']
				else 0,
			...
			farm_enterprises.append(
				{
					...
					'Value_of_Farm_Assets_NGN': round(
						value_of_farm_assets, 0
...
def create_loan_record_table(df_master_data, df_farm_enterprise, fake_instance):
	...
	farmer_to_first_enterprise = df_farm_enterprise.drop_duplicates(
		subset=['Farmer_ID'], keep='first'
	)
	farmer_to_first_enterprise = farmer_to_first_enterprise.set_index('Farmer_ID')[
		'Enterprise_ID'
	].to_dict()
	...
	for _, row in df_master_data.iterrows():
		...
		if row['has_prior_loan'] and row['prior_loan_amount_ngn'] > 0:
			...
			loan_asset_ratio = (
				(row['prior_loan_amount_ngn'] / enterprise_value_assets)
				if enterprise_value_assets > 0
				else 0
			)
			op_ex_income_ratio = (
				(enterprise_op_ex / enterprise_income) if enterprise_income > 0 else 0
			)

			has_defaulted = row['prior_loan_repayment_history'] == 'Poor'
			default_reason = (
				random.choice(['Market Loss', 'Low Yield', 'Illness', 'Diversion of Funds'])
				if has_defaulted
				else None
			)

			loan_source = random.choice(
				['Bank', 'Cooperative', 'BOA', 'Informal Lender', 'Microfinance']
			)
			...
			loan_records.append(
				{
					...
					'Interest_Rate_Annual': round(
						random.uniform(0.12, 0.40), 2
					),  # Annual interest rate
					'Loan_Date': loan_date_obj.strftime('%Y-%m-%d'),
					'Loan_Use_Status_As_Intended': random.choice([True, False]),
					'Loan_Repayment_Amount_Naira': round(
						row['prior_loan_amount_ngn'] * row['prior_loan_repayment_rate'], 0
					),
		...
		if row['requests_loan_now'] and row['loan_amount_requested_ngn'] > 0:
			...
			repayment_date_obj = loan_date_obj + relativedelta(
				months=row['current_loan_tenure_months']
			)

			loan_asset_ratio = (
				(row['loan_amount_requested_ngn'] / enterprise_value_assets)
				if enterprise_value_assets > 0
				else 0
			)
			op_ex_income_ratio = (
			...
			has_defaulted_predicted = (
				row['predicted_default_probability_current_loan'] > 0.4
			)  # Threshold for prediction
			default_reason_predicted = (
				random.choice(['Predicted Market Loss', 'Predicted Low Yield', 'Predicted Illness'])
				if has_defaulted_predicted
				else None
			)

			loan_source = random.choice(
				['Bank', 'Cooperative', 'BOA', 'Fintech Platform', 'Microfinance']
			)
			...
			loan_records.append(
				{
					...
					'Disbursement_Lag_Months': random.randint(
						0, 2
					),  # Typically shorter for new platforms
