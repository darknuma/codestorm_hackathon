...
def generate_farmer_data():
    ...
    data = {
        ...
        'education_years': np.random.normal(10, 2, NUMBER_OF_RECORDS).clip(0, 18).astype(int),
        ...
        'crop_type': np.random.choice(['Maize', 'Rice', 'Cassava', 'Yam'], NUMBER_OF_RECORDS),
    }
    
    farmer_df = pd.DataFrame(data)
    
    # Calculate loan repayment based on weighted conditions
    farmer_df['loan_repaid'] = (
        (
            farmer_df['loan_amount']
            * (1 - farmer_df['loan_diversion'])
            * (farmer_df['farm_income'] / farmer_df['loan_amount'])
            * np.where(farmer_df['disbursement_lag'] <= 3, 1, 0.8)
        )
        .clip(0, farmer_df['loan_amount'])
        .astype(int)
    )
    
    # Binary label for credit worthiness
    farmer_df['credit_worthy'] = (farmer_df['loan_repaid'] / farmer_df['loan_amount'] >= 0.5).astype(int)
