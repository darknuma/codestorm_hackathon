...
from uuid import uuid4 # Retained if needed for other unique ID generation, though farmer_id is custom
...
LIVESTOCK_WEIGHTS_BY_REGION = {
    ...
    'North West':    {'Poultry': 0.42, 'Goats': 0.72, 'Sheep': 0.56, 'Cattle': 0.48, 'Pigs': 0.01, 'Fish Farming': 0.01, 'Ducks': 0.03, 'Guinea Fowl': 0.05, 'None': 0.2}, # Highest livestock
    ...
    'South South':   {'Poultry': 0.53, 'Goats': 0.55, 'Sheep': 0.02, 'Cattle': 0.0, 'Pigs': 0.1, 'Fish Farming': 0.073, 'Ducks': 0.01, 'Guinea Fowl': 0.01, 'None': 0.4}, # Highest fishing
...
NATIONAL_LIVESTOCK_WEIGHTS = {'Poultry': 0.54, 'Goats': 0.65, 'Sheep': 0.31, 'Cattle': 0.25, 'Pigs': 0.05, 'Fish Farming': 0.02, 'Ducks': 0.03, 'Guinea Fowl': 0.02, 'None': 0.53}
...
LIVESTOCK_USE_WEIGHTS = [0.621, 0.207, 0.164, 0.098, 0.091, 0.010, 0.014] # Approx weights from Table 6.18

LAND_OWNERSHIP_STATUSES = ['Inherited', 'Family Owned', 'Purchased', 'Leased', 'Community Land', 'Rented', 'Gift'] # Added Inherited
...
LOAN_PRODUCTS_PLATFORM = ['Input Loan', 'Mechanization Loan', 'Bundled Services Loan'] # e.g., Zowasel
...
BASE_YIELDS_TONS_PER_HA = {
    ...
    'Cowpea': 1.0, 'Beans': 1.0, 'Groundnut': 1.2, 'Cotton': 1.5, 'Vegetables': 18.0, 'Oil Palm': 5.0,
    'Plantain': 12.0, 'Cocoa': 0.7, 'Rubber': 1.5, 'Soybean': 1.5, 'Sesame': 0.8, 'Tomato': 20.0,
...
CROP_PRICES_NAIRA_PER_TON = {
    ...
    'Rubber': 700000, 'Soybean': 380000, 'Sesame': 600000, 'Tomato': 250000,
    'Guinea Corn': 190000 # Alias for Sorghum
}
# Base cost per hectare (Naira) - simplified
BASE_COST_PER_HA = {crop: price * 0.50 for crop, price in CROP_PRICES_NAIRA_PER_TON.items()} # Adjusted cost ratio slightly
...
def calculate_yield_tons_per_ha(row):
    ...
    education_factor = {'No Formal Education': 0.8, 'Primary Incomplete': 0.85, 'Primary Complete': 0.9,
                        'Secondary Incomplete': 0.95, 'Secondary Complete': 1.0, 'OND/NCE': 1.1,
                        'HND/BSc': 1.15, 'Masters/PhD': 1.2}.get(row['education_category'], 1.0)

    modern_practices_factor = 1.0
    if row['uses_improved_seeds']: modern_practices_factor += 0.15 # Impact of improved seeds
    if row['uses_fertilizer']: modern_practices_factor += 0.20 # Impact of fertilizer
    if row['uses_irrigation']: modern_practices_factor += 0.35 # Higher impact for rare irrigation
    if row['pest_disease_control_method'] in ['Modern Chemical', 'Integrated Pest Management']: modern_practices_factor += 0.10
    if row['uses_extension_services']: modern_practices_factor += 0.05 # Small boost from extension
...
def calculate_annual_farm_expenses_ngn(row):
    ...
    if row['gender'] == 'Female' and row['farm_size_ha'] > 1.0: labor_factor += 0.05 # May hire more labor
...
def generate_off_farm_income_ngn(row):
    if row['has_off_farm_income']:
        education_base = {"No Formal Education": 80000, "Primary Incomplete": 100000, "Primary Complete": 150000,
                          "Secondary Incomplete": 200000, "Secondary Complete": 300000, "OND/NCE": 400000,
                          "HND/BSc": 500000, "Masters/PhD": 700000}.get(row['education_category'], 100000)
...
def generate_prior_loan_details(row):
    if row['has_prior_loan']:
        ...
        loan_amount = min(potential_revenue * random.uniform(0.10, 0.35), # Lower % of revenue
                          200000 + (row['farm_size_ha'] * 60000)) # Lower base + per ha amount
        ...
        if row['cooperative_member']: base_repayment_prob += 0.07 # Slightly higher impact
        if row['has_off_farm_income']: base_repayment_prob += 0.08 # Higher impact
        if row['farm_profit_ngn'] > loan_amount * 0.5: base_repayment_prob += 0.1 # Higher profit buffer needed

        repayment_prob = min(base_repayment_prob, 0.90)
        actual_repayment_rate = random.uniform(max(0.20, repayment_prob - 0.30), min(1.0, repayment_prob + 0.1)) # Wider variance
        ...
        elif actual_repayment_rate >= 0.55: repayment_history = "Fair" # Adjusted threshold
        else: repayment_history = "Poor"
    ...
    return pd.Series({'prior_loan_amount_ngn': 0, 'prior_loan_repayment_rate': 0,
                      'prior_loan_repayment_history': 'None', 'prior_loan_purpose': 'N/A'})

def calculate_credit_score(row): # Scale 300-850
    score = 350
    # Education
    edu_points = {"No Formal Education": 0, "Primary Incomplete": 10, "Primary Complete": 20,
                  "Secondary Incomplete": 30, "Secondary Complete": 50, "OND/NCE": 60,
                  "HND/BSc": 70, "Masters/PhD": 80}
    ...
    if row['has_off_farm_income']: score += 50 # Increased impact
    if row['yield_consistency_rating'] == 'High': score += 20
    elif row['yield_consistency_rating'] == 'Medium': score += 10
    elif row['yield_consistency_rating'] == 'Low': score -= 10 # Penalty for low consistency
    ...
    if row['has_land_title']: score += 25 # Bonus for title
    # Prior Loan History
    if row['has_prior_loan']:
        history_pts = {'Excellent': 70, 'Good': 40, 'Fair': -10, 'Poor': -70, 'None':0} # Increased penalty
        score += history_pts.get(row['prior_loan_repayment_history'], 0)
        score += int((row['prior_loan_repayment_rate'] - 0.6) * 100) # Adjusted baseline, scale -48 to +32
    # Social Capital
    if row['cooperative_member']: score += 40 # Increased impact
    if row['uses_extension_services']: score += 15 # Added extension service impact
    ...
    if row['uses_irrigation']: score += 15 # Irrigation still valuable if present
    # Risk Mitigation
    if row['has_weather_insurance']: score += 35 # Higher impact for rare insurance
    if row['has_storage_facility']: score += 15
    # Digital Footprint
    if row['smartphone_owner']: score += 20
    score += (row['digital_footprint_score_1_10'] -1) * 2 # Max 18
    if row['value_chain_platform_registered'] and row['years_on_platform'] > 0 : score += 30 # Increased impact
    # Profitability
    profit_margin = row['farm_profit_ngn'] / max(row['annual_farm_revenue_ngn'], 1)
    if profit_margin > 0.30: score += 35 # Adjusted thresholds
    elif profit_margin > 0.15: score += 20
    elif profit_margin > 0.0: score += 5
    else: score -= 30 # Increased penalty
...
def calculate_predicted_default_probability(row): # For current loan request
    ...
    experience_factor = max(0.70, 1.0 - (row['farming_experience_years'] * 0.02)) # More impact from experience

    modern_practices_factor = 1.0
    if row['uses_improved_seeds']: modern_practices_factor -= 0.05 # Slightly more impact
    ...
    social_factor = social_factor * 0.95 if row['uses_extension_services'] else social_factor # Extension impact

    prior_history_factor = 1.0
    if row['has_prior_loan']:
        history_map = {'Excellent': 0.65, 'Good': 0.80, 'Fair': 1.05, 'Poor': 1.35, 'None': 1.0} # Adjusted map
    ...
    capacity_factor = 1.0 + min(max(0, loan_to_income_ratio - 0.25) * 2.5, 0.6) # Penalize earlier, higher penalty

    default_prob = base_risk * credit_factor * income_stability_factor * experience_factor * \
                   modern_practices_factor * social_factor * prior_history_factor * capacity_factor
...
for i in range(NUM_FARMERS):
    ...
    gender = random.choices(['Male', 'Female'], weights=[0.75, 0.25])[0] # Reflecting HOH stats more

    education_category = weighted_choice([
        ("No Formal Education", 0.25), ("Primary Incomplete", 0.15), ("Primary Complete", 0.25), # Higher no/primary
        ("Secondary Incomplete", 0.10), ("Secondary Complete", 0.18), ("OND/NCE", 0.04),
        ("HND/BSc", 0.02), ("Masters/PhD", 0.01) # Lower tertiary
    ])
    education_level_years = EDUCATION_LEVELS_MAPPING[education_category]

    marital_status = weighted_choice([
        ('Single', 0.15 if age < 30 else 0.05), ('Married', 0.75), # Higher married rate
        ('Divorced', 0.03), ('Widowed', 0.07)
    ])
    household_size = random.randint(1, 5) if marital_status == 'Single' and age < 30 else random.randint(3, 14) # Typical range 4-6 mentioned

    state = random.choice(NIGERIAN_STATES)
    region = STATE_TO_REGION[state]

    has_off_farm_income = random.choices([True, False], weights=[0.45, 0.55])[0] # Slightly lower off-farm
    ...
    farm_size_ha = round(farm_size_min + (farm_size_max - farm_size_min) * (random.random()**2.5), 2) # Power concentrates smaller values
    ...
    if gender == 'Female':
        uses_fertilizer = uses_fertilizer if random.random() < 0.9 else False # Slightly less likely
    ...
    has_weather_insurance = random.choices([True, False], weights=[0.05, 0.95])[0] # Even lower insurance rate

    # Land Tenure
    land_acquisition_method = weighted_choice([
        ('Inheritance', 0.65), ('Family Allocation', 0.15), ('Purchase', 0.08), # ~10% purchase overall
        ('Lease', 0.05), ('Rental', 0.03), ('Community Grant', 0.02), ('Gift', 0.02)
    ])
    # Lower title probability based on context
    has_land_title = False
    if land_acquisition_method == 'Purchase':
        has_land_title = random.choices([True, False], weights=[0.20, 0.80])[0] # Higher chance if purchased
    else:
        has_land_title = random.choices([True, False], weights=[0.05, 0.95])[0] # Low chance otherwise
    ...
    has_bank_account = random.choices([True, False], weights=[0.55, 0.45])[0] # Slightly lower formal access
    has_formal_id = random.choices([True, False], weights=[0.65, 0.35])[0]
    smartphone_owner = random.choices([True, False], weights=[0.60, 0.40])[0] # Lower smartphone penetration assumed
    mobile_money_usage_frequency = "Never"
    if smartphone_owner:
        mobile_money_usage_frequency = weighted_choice([
            ('Daily', 0.10), ('Weekly', 0.30), ('Monthly', 0.35), ('Rarely', 0.20), ('Never', 0.05)
        ])
    ...
    value_chain_platform_registered = random.choices([True, False], weights=[0.25, 0.75])[0] # Lower registration rate
    years_on_platform = 0
    ...
    if value_chain_platform_registered:
        ...
        if years_on_platform > 0:
            marketplace_sales_ngn_last_year = random.uniform(0.1, 0.6) # Factor of potential revenue

    # Cooperative
    cooperative_member = random.choices([True, False], weights=[0.40, 0.60])[0] # Keep as is, distinct from extension
    ...
    temp_row_for_calc = {
        ...
        'gender': gender, 'uses_extension_services': uses_extension_services # Pass gender for expense calc
    }
    ...
    crop_disposition = {'Stored': 0.40, 'Sold Unprocessed': 0.25, 'Sold Processed': 0.05, 'Consumed': 0.25, 'Given Out': 0.04, 'Lost': 0.01} # Generic average
    if primary_crop == 'Cassava': crop_disposition = {'Stored': 0.10, 'Sold Unprocessed': 0.28, 'Sold Processed': 0.17, 'Consumed': 0.36, 'Given Out': 0.06, 'Lost': 0.005}
    elif primary_crop == 'Yam': crop_disposition = {'Stored': 0.41, 'Sold Unprocessed': 0.20, 'Sold Processed': 0.01, 'Consumed': 0.31, 'Given Out': 0.05, 'Lost': 0.01}
    elif primary_crop == 'Maize': crop_disposition = {'Stored': 0.41, 'Sold Unprocessed': 0.25, 'Sold Processed': 0.00, 'Consumed': 0.24, 'Given Out': 0.05, 'Lost': 0.003}
    elif primary_crop in ['Sorghum', 'Millet', 'Guinea Corn']: crop_disposition = {'Stored': 0.66, 'Sold Unprocessed': 0.07, 'Sold Processed': 0.00, 'Consumed': 0.19, 'Given Out': 0.04, 'Lost': 0.001}
    elif primary_crop == 'Rice': crop_disposition = {'Stored': 0.56, 'Sold Unprocessed': 0.27, 'Sold Processed': 0.01, 'Consumed': 0.10, 'Given Out': 0.03, 'Lost': 0.001}
    elif primary_crop == 'Groundnut': crop_disposition = {'Stored': 0.54, 'Sold Unprocessed': 0.32, 'Sold Processed': 0.01, 'Consumed': 0.09, 'Given Out': 0.03, 'Lost': 0.001}
    elif primary_crop in ['Cowpea', 'Beans']: crop_disposition = {'Stored': 0.47, 'Sold Unprocessed': 0.22, 'Sold Processed': 0.01, 'Consumed': 0.24, 'Given Out': 0.03, 'Lost': 0.001}
    ...
    prior_loan_data = generate_prior_loan_details({**temp_row_for_calc,
    ...
    requests_loan_now = random.choices([True, False], weights=[0.5, 0.5])[0] # Lower request rate maybe
    ...
    if requests_loan_now:
        ...
        loan_amount_requested_ngn = round(random.uniform(20000, max(50000, max_sensible_request * 1.5)) / 5000,0) * 5000 # Round to 5k
    ...
    full_row_for_scoring = {
        'education_category': education_category, 'has_bank_account': has_bank_account,
        'has_formal_id': has_formal_id, 'mobile_money_usage_frequency': mobile_money_usage_frequency,
        'farming_experience_years': farming_experience_years, 'has_off_farm_income': has_off_farm_income,
        ...
        'land_acquisition_method': land_acquisition_method, 'has_land_title': has_land_title,
        'has_prior_loan': has_prior_loan,
        ...
        'owns_tractor': owns_tractor, 'owns_plow': owns_plow, 'owns_sprayer': owns_sprayer
    }
    ...
    suitable_loan_products = determine_suitable_loan_products({**full_row_for_scoring, 'credit_score': credit_score, 'farm_size_ha': farm_size_ha})

    predicted_default_prob = 0.0
    predicted_loan_repayment_outcome = "N/A"
    if requests_loan_now:
        predicted_default_prob = calculate_predicted_default_probability({**full_row_for_scoring, 'credit_score': credit_score})
    ...
    farm_size_category = pd.cut([farm_size_ha], bins=[0, 1, 5, 10, float('inf')],
